...
Como se pode ver no _output_ do comando `ssh-audit`, neste servidor é usada a versão 7.4p1 do OpenSSH. Segundo os dados obtidos pelo _output_, os algoritmos de chave, cifragem, acordo de chaves e de _MAC_ são seguros.

### Pergunta 3.1
#### Eurotux
A primeira empresa escolhida foi a [_Eurotux_](https://eurotux.com). A informação relativa a um dos seus servidores _ssh_ pode ser encontrada [aqui](https://www.shodan.io/host/194.107.127.25). Correndo a ferramenta `ssh-audit` para o servidor 194.107.127.25 encontrado com o _shodan_, obtém-se o seguinte output:
```
# general
...
Como se pode ver pelo _output_ do `ssh-audit`, o servidor corre a versão 6.0p1 do _OpenSSH_.

### Nutrium
A segunda empresa escolhida foi a [Nutrium](nutrium.io). Procurando por `nutrium.io` no _shodan_, obtém-se o [endereço de um dos seus servidores _ssh_](https://www.shodan.io/host/40.127.196.98). Analisando este endereço com o `ssh-audit` obtém-se o seguinte output:
```
# general
